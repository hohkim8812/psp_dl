...
class StyledGenerator(nn.Module):
    ...
    def forward(
        ...
        embedding = self.linear1(label.unsqueeze(1))
        ...
        for i in input:
            new_i = torch.cat([i,embedding], dim=1)
            new_i = self.linear2(new_i)
            styles.append(self.style(new_i))
...
class Discriminator(nn.Module):
    def __init__(self, fused=True, from_rgb_activate=False):
        ...
        def make_from_rgb(out_channel):
            if from_rgb_activate:
                return nn.Sequential(EqualConv2d(3+1, out_channel, 1), nn.LeakyReLU(0.2))
    ...
    def forward(self, input, label, step=0, alpha=-1):
        ...
        h,w = input_size[2:]
        label = label.view(-1,1,1,1).repeat(1,1,h,w)
