...
def train(args, dataset, generator, discriminator):
    ...
    for i in pbar:
        ...
        if (i + 1) % 100 == 0:
            ...
            gen_i, gen_j = args.gen_sample.get(resolution, (105, 9))
            test_label = torch.tensor([0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0], dtype=torch.float32).cuda()
            print(test_label)
            with torch.no_grad():
                for _ in range(gen_i):
                    images.append(
                        g_running(
                            torch.randn(gen_j, code_size).cuda(), test_label, step=step, alpha=alpha
                        ).data.cpu()
                    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label_path', type=str, help='path of specified label dataset')
    parser.add_argument(
        '--phase',
        type=int,
        default=600_000,
    ...
    dataset = MultiResolutionDataset(args.img_path, args.label_path, transform)
