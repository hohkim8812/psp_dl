...
def save_images(folder_path, output_root):
    ...
    for file in image_files:
        ...
        for i in range(0, img.width, 34):
            for j in range(0, img.height, 33):
...
def prepare(transaction1, transaction2, dataset, n_worker, sizes=(8, 16, 32, 64, 128)):
    ...
    for file in files:
        ...
        for size, img in zip(sizes, imgs):
            ...
            key = f'{size}-{str(i).zfill(5)}'.encode('utf-8')
            transaction1.put(key, img)
            transaction2.put(key, label)
            total += 1
    total = total//5 # /5 for 5 image fol
    transaction1.put('length'.encode('utf-8'), str(total).encode('utf-8')) # /5 for 5 image fol
    transaction2.put('length'.encode('utf-8'), str(total).encode('utf-8'))

if __name__ == '__main__':

    parser = argparse.ArgumentParser()
    parser.add_argument('--folder_path', type=str, default='../extended_tensile_test_images')
    parser.add_argument('--output_root', type=str, default='images/128x128')
    ...
    parser.add_argument('--img_src_path', type=str, default='images/128x128')
